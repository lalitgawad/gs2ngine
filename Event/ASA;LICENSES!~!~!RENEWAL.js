//REVISIT - Whole Script need to revisit
...
logDebug("paremt"+parentCapId+isRenewProcess(parentCapId, partialCapId)+"partialcapid"+partialCapId);
//1. Check to see if license is ready for renew

//Added below line to restrict below code from executing at Renewal application Submission
var applicantCheck = gs2.rec.renewalContactTypeChecking(partialCapId);
var readyForRenew=isReadyRenew(parentCapId);
if (readyForRenew && !applicantCheck)
{
...
function copyKeyInfo(srcCapId, targetCapId)
{
	...
	var cap = aa.cap.getCap(srcCapId).getOutput();
	var appTypeResult = cap.getCapType().toString().toUpperCase();
	copyApplicationSpecificInfo(srcCapId, targetCapId);
	...
	if (s_result.getSuccess())
	{
		...
		if (capPeopleArr != null || capPeopleArr.length > 0)
		{
			for (loopk in capPeopleArr)
			{
				...
				var refContactNum = capContactModel.refContactNumber;

				logDebug("capContactModel.getContactType() " + capContactModel.getContactType());
				if(capContactModel.getContactType() == "Licensed Individual" || capContactModel.getContactType() == "Business")
				{
					var contactAddressrs = new Array();
					var fvContactAddress = getLatestContactAddress(capContactModel, "Mailing Address");

					// Copy the most recent contact address
					contactAddressrs.push(createRefContactAddressFromAddress(refContactNum, fvContactAddress, "Mailing Address"));

					var contactAddressModelArr = convertContactAddressModelArr(contactAddressrs);
...
function copyApplicationName(srcCapId, targetCapId){
	var capResult = aa.cap.getCap(srcCapId);
	var cap = capResult.getOutput();
	var appName = cap.getSpecialText();
	editAppName(appName, targetCapId);
}
