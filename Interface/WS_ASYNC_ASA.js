/*------------------------------------------------------------------------------------------------------/
| Program : WS_ASYNC_ASA.js
| Event   : N/A
| Usage   : Custom EMSE Script to run from ASA as aync
...
function stratAsaLogic() {
    ...
    try {
        ...
        try {
            logDebug("validateRecordType--> "+validateRecordType());
            if (!validateRecordType()) {
                sendMultipleEmailsAsa2AA();
            }   

            sendMultipleEmailsAsa3AA();
        } catch (err) {
            logDebug("A JavaScript Error occurred: #validateRecordType: " + err.message);
        }


        try {
            if (appObj) {
                appObj.AsaAsyncDelegator();
            }
        } catch (err) {
        ...
        try {
            ...
            logDebug("**WARNING: ASA:LandDevelopment/*/*/*: #ID-CreateEdrefSubTskWrapper:  " + err.message);
        }

        //Add if statement for Parks if Commercial WS_ASYNC_ASA
        try {
            ...
            logDebug("WARNING JavaScript Error occurred: ASA LandDevelopment WS_ASYNC_ASA: " + err.message);
        }

		//Default Under Review Status Workflow History Function ASA LandDevelopment WS_ASYNC_ASA
		try {
			if (!publicUser) {
...
function validateRecordType()
	{
		...
		logDebug("WS_ASYNC_ASA >>> validateRecordType NOTIFICATIONLIST --> "+notificationList)
